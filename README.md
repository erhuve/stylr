...
Presentation audit: all 6,899 repository images reviewed; 1,160 masculine, 3,896 feminine, 163 mixed/androgynous and 1,680 unclear outfits. `data/presentation/coverage.md` records source/build coverage and complete references. These are styling observations, not gender or unique-model counts. Future sourcing prioritizes extensive coverage of both presentations, particularly fuller-build masculine outfits with visible contours; exact parity is not required.

Current repository catalog (2026-09-17): 6,899 photos and 285 complete three-axis references. Seven reviewed batches add 1,793 photos from 2,467 candidates, including 24 complete references. The broader-build benchmark rises from 52 to 54 matches; the latest 152 additions provide no new complete references or benchmark matches. The requested doubling target is 10,212 admitted photos and remains in progress. These additions are not deployed: the last verified private live release remains 3,139 photos and 202 complete references. `docs/verification/doubling-sourcing.md` is the active continuation; `docs/verification/postdeploy-sourcing.md` records the live release. `data/sourcing/README.md` documents sampled and deferred pages, provenance and source yields. Offline sourcing and clustering code lives in `research/`; see `research/README.md`.
...
The repository library contains **6,899 color references**: the original 385 unchanged photographs plus 6,514 admitted outfit photographs from 9,262 reviewed candidates. The latest archive/retail continuation adds 26 masculine-styled, 74 feminine-styled, two mixed and 50 unclear outfits; hidden body traits remain unknown. The old 831-image archive backlog is fully reviewed. The remaining 3,960 training-archive candidates are explicitly pending and excluded from admission. Reviewed partial and seated photos are retained as detail views. The interface preloads only the current and two upcoming images. See `docs/catalog-pipeline.md` for sourcing, visual review, research history, reproducible admission and repository organization.
...
The body controls use 6,658 direct visual review records: 144 existing reviews plus 6,514 admitted expansion records. 5,399 have apparent-build observations and 285 support all three required axes. The broader-build / similar shoulders and hips / moderate-waist benchmark has 54 repository matches, versus 38 in the last verified deployed release. The current user's exact slider values have not been confirmed, so this benchmark is not a claim about their selection. Labels do not use the research model's predictions. Shape filters reduce these counts; empty combinations stay empty. These are approximate visual judgments, not measurements or clothing sizes. See `data/apparent-build/README.md` and `data/catalog-review/admission-summary.json`. Saved favorites outside the selected filters are hidden without deleting their reactions.
