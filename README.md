...
The repository library contains **1,872 color references**: the original 385 unchanged photographs plus 1,487 admitted outfit photographs from 2,011 reviewed candidates. The latest three batches reviewed 380 new candidates, admitting 165 photos across 143 new source pages and four complete three-axis references. Reviewed partial and seated photos are retained as detail views. The interface preloads only the current and two upcoming images. See `docs/catalog-pipeline.md` for sourcing, visual review, research history, reproducible admission and repository organization.
...
The body controls use 1,631 direct visual review records: 144 existing reviews plus 1,487 admitted expansion records. 1,396 have apparent-build observations and 199 support all three required axes. The broader-build / similar shoulders and hips / moderate-waist example has 37 matches in the repository, up from the deployed 33. They do not use the research model's predictions. Shape filters reduce these counts; empty combinations stay empty. These are approximate visual judgments, not measurements or clothing sizes. See `data/apparent-build/README.md` and `data/catalog-review/admission-summary.json`. Saved favorites outside the selected filters are hidden without deleting their reactions.
