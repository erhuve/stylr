...
Presentation audit: all 5,106 repository images reviewed; 849 masculine, 2,873 feminine, 127 mixed/androgynous and 1,257 unclear outfits. `data/presentation/coverage.md` records source/build coverage and complete references. These are styling observations, not gender or unique-model counts. Future sourcing prioritizes extensive coverage of both presentations, particularly fuller-build masculine outfits with visible contours; exact parity is not required.

Current repository catalog (2026-09-17): 5,106 photos and 261 complete three-axis references. The latest two batches add 47 photos from 146 reviewed candidates, including six complete references and two new publishers. The broader-build benchmark rises from 51 to 52 matches. These additions are not deployed: the last verified private live release remains 3,139 photos and 202 complete references. `docs/verification/fitted-sourcing-continuation.md` records this continuation; `docs/verification/five-thousand-sourcing.md` records the prior expansion and pending archive; `docs/verification/postdeploy-sourcing.md` records the live release. `data/sourcing/README.md` documents sampled and deferred pages, provenance and source yields. Offline sourcing and clustering code lives in `research/`; see `research/README.md`.
...
The repository library contains **5,106 color references**: the original 385 unchanged photographs plus 4,721 admitted outfit photographs from 6,795 reviewed candidates. The latest continuation adds 19 masculine-styled, 10 feminine-styled and 18 unclear outfits; hidden body traits remain unknown. Another 831 downloaded archive candidates are explicitly unreviewed and not admitted. Reviewed partial and seated photos are retained as detail views. The interface preloads only the current and two upcoming images. See `docs/catalog-pipeline.md` for sourcing, visual review, research history, reproducible admission and repository organization.
...
The body controls use 4,865 direct visual review records: 144 existing reviews plus 4,721 admitted expansion records. 3,732 have apparent-build observations and 261 support all three required axes. The broader-build / similar shoulders and hips / moderate-waist benchmark has 52 repository matches, versus 38 in the last verified deployed release. The current user's exact slider values have not been confirmed, so this benchmark is not a claim about their selection. Labels do not use the research model's predictions. Shape filters reduce these counts; empty combinations stay empty. These are approximate visual judgments, not measurements or clothing sizes. See `data/apparent-build/README.md` and `data/catalog-review/admission-summary.json`. Saved favorites outside the selected filters are hidden without deleting their reactions.
