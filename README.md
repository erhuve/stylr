...
The repository library contains **7,068 color references**: the original 385 unchanged photographs plus 6,683 admitted outfit photographs from 9,580 reviewed candidates. The latest archive/retail continuation adds 11 masculine-styled, 32 feminine-styled, two mixed and 16 unclear outfits; hidden body traits remain unknown. The old 831-image archive backlog is fully reviewed. The remaining 3,816 training-archive candidates are explicitly pending and excluded from admission. Reviewed partial and seated photos are retained as detail views. The interface preloads only the current and two upcoming images. See `docs/catalog-pipeline.md` for sourcing, visual review, research history, reproducible admission and repository organization.
...
The body controls use 6,827 direct visual review records: 144 existing reviews plus 6,683 admitted expansion records. 5,550 have apparent-build observations and 289 support all three required axes. The broader-build / similar shoulders and hips / moderate-waist benchmark has 54 repository matches, versus 38 in the last verified deployed release. The current user's exact slider values have not been confirmed, so this benchmark is not a claim about their selection. Labels do not use the research model's predictions. Shape filters reduce these counts; empty combinations stay empty. These are approximate visual judgments, not measurements or clothing sizes. See `data/apparent-build/README.md` and `data/catalog-review/admission-summary.json`. Saved favorites outside the selected filters are hidden without deleting their reactions.
