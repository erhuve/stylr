...
Presentation audit: all 6,123 repository images reviewed; 985 masculine, 3,555 feminine, 153 mixed/androgynous and 1,430 unclear outfits. `data/presentation/coverage.md` records source/build coverage and complete references. These are styling observations, not gender or unique-model counts. Future sourcing prioritizes extensive coverage of both presentations, particularly fuller-build masculine outfits with visible contours; exact parity is not required.

Current repository catalog (2026-09-17): 6,123 photos and 283 complete three-axis references. Four reviewed batches add 1,017 photos from 1,507 candidates, including 22 complete references. The broader-build benchmark rises from 52 to 54 matches. The requested doubling target is 10,212 admitted photos and remains in progress. These additions are not deployed: the last verified private live release remains 3,139 photos and 202 complete references. `docs/verification/doubling-sourcing.md` is the active continuation; `docs/verification/postdeploy-sourcing.md` records the live release. `data/sourcing/README.md` documents sampled and deferred pages, provenance and source yields. Offline sourcing and clustering code lives in `research/`; see `research/README.md`.
...
The repository library contains **6,123 color references**: the original 385 unchanged photographs plus 5,738 admitted outfit photographs from 8,302 reviewed candidates. The latest continuation adds 136 masculine-styled, 682 feminine-styled, 26 mixed and 173 unclear outfits; hidden body traits remain unknown. The old 831-image archive backlog is fully reviewed. New training-archive intake remains separate and unreviewed until inspected. Reviewed partial and seated photos are retained as detail views. The interface preloads only the current and two upcoming images. See `docs/catalog-pipeline.md` for sourcing, visual review, research history, reproducible admission and repository organization.
...
The body controls use 5,882 direct visual review records: 144 existing reviews plus 5,738 admitted expansion records. 4,674 have apparent-build observations and 283 support all three required axes. The broader-build / similar shoulders and hips / moderate-waist benchmark has 54 repository matches, versus 38 in the last verified deployed release. The current user's exact slider values have not been confirmed, so this benchmark is not a claim about their selection. Labels do not use the research model's predictions. Shape filters reduce these counts; empty combinations stay empty. These are approximate visual judgments, not measurements or clothing sizes. See `data/apparent-build/README.md` and `data/catalog-review/admission-summary.json`. Saved favorites outside the selected filters are hidden without deleting their reactions.
