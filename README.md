...
Presentation audit: all 3,139 repository images reviewed; 664 masculine, 1,457 feminine, 112 mixed/androgynous and 906 unclear outfits. `data/presentation/coverage.md` records source/build coverage and complete references. These are styling observations, not gender or unique-model counts. Future sourcing prioritizes extensive coverage of both presentations, particularly fuller-build masculine outfits with visible contours; exact parity is not required.

Current repository and private live catalog (2026-09-16, post-deployment continuation): 3,139 photos and 202 complete three-axis references. After deploying the 3,086-photo release, 117 additional candidates yielded 53 photos from ONE BONE, Wildfang, Tombolo and Snag; these additions are also privately deployed. The broader-build example remains at 38 matches. `docs/verification/postdeploy-sourcing.md` records the latest verification and deployment status. `data/sourcing/README.md` documents sampled and deferred pages, provenance and source yields. The earlier archive pass is recorded in `docs/verification/three-thousand-sourcing.md`. Offline sourcing and clustering code lives in `research/`; see `research/README.md`.
...
The repository library contains **3,139 color references**: the original 385 unchanged photographs plus 2,754 admitted outfit photographs from 3,657 reviewed candidates. The latest continuation adds 53 photos across 46 new source pages, 50 build observations and no complete three-axis references. It adds 22 masculine-styled, 19 feminine-styled and 12 neutral/unclear outfits; hidden body traits remain unknown. Reviewed partial and seated photos are retained as detail views. The interface preloads only the current and two upcoming images. See `docs/catalog-pipeline.md` for sourcing, visual review, research history, reproducible admission and repository organization.
...
The body controls use 2,898 direct visual review records: 144 existing reviews plus 2,754 admitted expansion records. 1,913 have apparent-build observations and 202 support all three required axes. The broader-build / similar shoulders and hips / moderate-waist example has 38 matches in both the repository and deployed release. They do not use the research model's predictions. Shape filters reduce these counts; empty combinations stay empty. These are approximate visual judgments, not measurements or clothing sizes. See `data/apparent-build/README.md` and `data/catalog-review/admission-summary.json`. Saved favorites outside the selected filters are hidden without deleting their reactions.
