...
4. **The existing public Site builds directly from its working directory.** Moved this iteration to a separate Site at `Sites/stylr-personal`, initially unpublished and subsequently privately published with approval. Restored `Sites/stylr` to `origin/main` baseline `160d070` and restored its matching public bundle. The personal production directory now also serves its own `dist`; future pre-release builds must use an isolated checkout.
...
**Runtime issue resolved by approved private deployment (2026-09-14):** the development-preview runtime stopped during host restarts. Rather than depending on reopening that runtime, published the Site privately at https://stylr-personal-hatsunemiku.zo.computer/. Its managed production service is independent of the dev process. Six tests passed against the actual production listener with build interception disabled: four viewport/above-the-fold checks plus real mouse and touch swipes. Unauthenticated HTTPS requests to `/`, `/illustrated` and `/photos/archive-10000.webp` all returned 302 redirects to Zo sign-in. The earlier failed dev-runtime run is not counted as passing.

The deployment used source commit `ad46303`; subsequent changes are documentation-only. The previous public URL was rechecked against its unchanged baseline bundle (`index-B9zEd6yH.js`), and its working directory remains clean. Different browser origins have separate localStorage: existing development-preview or old-public-site reactions do not automatically move to the private published URL. The v2 format and image identities remain unchanged.
