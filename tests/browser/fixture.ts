    ...
    if (!process.env.TEST_BASE_URL) {
      ...
      await context.route('https://stylr.test/**', async route => {
        ...
        try {
          ...
          const contentType = ({ '.js': 'application/javascript', '.css': 'text/css', '.html': 'text/html', '.svg': 'image/svg+xml', '.woff2': 'font/woff2', '.png': 'image/png' } as Record<string, string>)[path.extname(target)] || 'application/octet-stream';
